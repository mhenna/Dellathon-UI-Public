...
export class UserService {
  ...
  registerParticipants(participants, teamName, emails, numberOfMembers):Promise<any>{
    return new Promise((resolve, reject) => {
      ...
      reqHeaders.append('Content-Type', 'application/json');
      reqHeaders.append('Access-Control-Allow-Origin', '*');
      this.http.post("http://localhost:3200/addUser", {'participants':participants, 'teamName':teamName, 'emails':emails, 'numberOfMembers':numberOfMembers})
      .subscribe((data) => resolve(data), err => reject(err));
    });
  }
  ...
  idExists(nationalID):Promise<any>{
    return new Promise((resolve, reject) => {
      ...
      reqHeaders.append('Content-Type', 'application/json');
      reqHeaders.append('Access-Control-Allow-Origin', '*');
      this.http.post("http://localhost:3200/idExists", {'nationalID':nationalID})
      .subscribe((data) => resolve(data), err => reject(err));
    });
  }
